...
class TCPHandler extends FrameHandler
{
    ...
    protected function getData(): array
    {
        ...
        return [
            ...
            self::FLAG_NS => $user_flagNs === convertAndFormatBin($this->frameComponent->getTcpflags()->getEcn(), 1) ? 1 : 0,
            self::FLAG_CWR => $user_flagCwr === convertAndFormatBin($this->frameComponent->getTcpflags()->getEcn(), 1) ? 1 : 0,
            self::FLAG_ECE => $user_flagEce === convertAndFormatBin($this->frameComponent->getTcpflags()->getEce(), 1) ? 1 : 0,
            self::FLAG_URG => $user_flagUrg === convertAndFormatBin($this->frameComponent->getTcpflags()->getUrgent(), 1) ? 1 : 0,
            self::FLAG_ACK => $user_flagAck === convertAndFormatBin($this->frameComponent->getTcpflags()->getAcknowledge(), 1) ? 1 : 0,
            self::FLAG_PSH => $user_flagPsh === convertAndFormatBin($this->frameComponent->getTcpflags()->getPush(), 1) ? 1 : 0,
            self::FLAG_RST =>  $user_flagRst === convertAndFormatBin($this->frameComponent->getTcpflags()->getReset(), 1) ? 1 : 0,
            self::FLAG_SYN => $user_flagSyn === convertAndFormatBin($this->frameComponent->getTcpflags()->getSyn(), 1) ? 1 : 0,
            self::FLAG_FIN => $user_flagFin === convertAndFormatBin($this->frameComponent->getTcpflags()->getFin(), 1) ? 1 : 0
        ];
